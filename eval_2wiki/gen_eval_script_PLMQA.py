...
MODEL_PATH=<parent folder name for <save directory>>/{}
DATA_PATH=dataset_2wiki_0.1/QA/
...
for checkpoint_step in all_checkpoint_steps:
    ...
    to_write = script.format(model_path+path_suffix, model_path+path_suffix)
    ...
    g2.write('python eval_qa.py --predict_file <location for saving prediction files>/{}predictions_valid.txt --eval_type qa\n'
             .format(model_path + path_suffix))

    g3.write('echo {}\n'.format(checkpoint_step))
    g3.write('python eval_qa.py --predict_file <location for saving prediction files>/{}predictions_test.txt --eval_type qa\n'
             .format(model_path + path_suffix))

file_name_sh = model_path.strip("/")+"-{}.sh".format("")
to_write = script.format(model_path, model_path)
f1 = open(file_name_sh, "w", encoding='utf-8')
f1.write(to_write)
f1.close()
g.write("sbatch "+file_name_sh)
