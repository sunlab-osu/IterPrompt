...
class GenerationMixin:
    ...
    def generate(
        ...
        use_casual_decoder_embeddings=False,  ####
        insert_decoder_prompt=False,
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            return self.greedy_search(
                ...
                use_casual_decoder_embeddings=use_casual_decoder_embeddings,  ####
                insert_decoder_prompt=insert_decoder_prompt,
                **model_kwargs,
            )
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        # TODO: here it only works with past=True; perhaps change this later
        if insert_decoder_prompt:
            assert type(self.n_tokens_decoder) == int
            assert type(self.pt_token_id) == int
            input_ids = torch.cat(
                [input_ids, torch.full((input_ids.shape[0], self.n_tokens_decoder), self.pt_token_id, dtype=torch.int64).to(input_ids.device)], 1)
        while True:
            if synced_gpus:
            ...
            # model_kwargs['past'] = None      #### no cutting past generated tokens for input TODO: perhaps optimize this
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            # model_inputs.keys(): dict_keys(['input_ids', 'encoder_outputs', 'past_key_values', 'decoder_input_ids', 'attention_mask', 'head_mask', 'decoder_head_mask', 'cross_attn_head_mask', 'use_cache'])
            # model_inputs['encoder_outputs'].last_hidden_state: fixed, torch.Size([batch_size, input_seq_len, 1024])
            # print("decoder_input_ids:", model_inputs['decoder_input_ids'][0])  # [id of last generated token = input_ids[0][-1]]
            #for key in ['past_key_values', 'attention_mask', 'head_mask', 'decoder_head_mask', 'cross_attn_head_mask', 'use_cache']:
            ...
            # print("input_ids[0]", input_ids[0])  list of all generated tokens
        ...
        if insert_decoder_prompt:
            filter_columns = [0]
            for qqq in range(self.n_tokens_decoder+1, input_ids.shape[1]):
                filter_columns.append(qqq)
            input_ids = input_ids[:, filter_columns]
